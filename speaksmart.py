...
# --- ИЗМЕНЕНИЕ 1: ОСНОВНАЯ КЛАВИАТУРА МЕНЮ ---
# Теперь здесь только одна кнопка.
main_menu_layout = [
    [KeyboardButton("новый текст")]
]
# one_time_keyboard=False означает, что клавиатура будет постоянной.
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text(
        text=welcome_text,
        reply_markup=main_menu_keyboard # Показываем нашу новую клавиатуру
    )

# --- ИЗМЕНЕНИЕ 4.1: ОБНОВЛЕННАЯ ФУНКЦИЯ ДЛЯ ЗАПРОСА ТЕКСТА ---
async def request_text_for_correction(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Запрашивает у пользователя текст для исправления после нажатия кнопки."""
    # Эта функция теперь запускается сообщением, а не callback_query
...
# --- ИЗМЕНЕНИЕ 3: ОБНОВЛЕНИЕ МЕНЮ ПОСТОБРАБОТКИ ---
async def _send_post_processing_menu(update_or_query, context: ContextTypes.DEFAULT_TYPE, response_text: str, message_prefix: str):
    ...
    # ... (весь код для форматирования текста остается без изменений) ...
    ...
    # Убираем кнопку "Новый текст" из этого меню
    ...
    # ... (вся логика отправки и обработки ошибок остается без изменений) ...
    try:
...
# --- ИЗМЕНЕНИЕ 4.2: ОБНОВЛЕННАЯ ФУНКЦИЯ ПОСТОБРАБОТКИ ---
async def post_processing_action(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # Логика для "start_new_text" полностью удалена, так как кнопки больше нет.
    # Пользователь будет использовать основную кнопку "новый текст".
...
async def cancel_conversation(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if update.callback_query:
        ...
        # Сообщение теперь другое, так как нет команды /start в меню
        ...
        await update.message.reply_text("Действие отменено. Чтобы начать заново, нажми «новый текст».", 
                                      reply_markup=main_menu_keyboard) # Убедимся что клавиатура на месте
...
def main() -> None:
    ...
    # --- ИЗМЕНЕНИЕ 4.3: ОБНОВЛЕННЫЙ CONVERSATIONHANDLER ---
    conv_handler = ConversationHandler(
        # Точка входа теперь - сообщение с текстом "новый текст"
        entry_points=[MessageHandler(filters.Text(["новый текст"]), request_text_for_correction)],
        states={
            # Фильтр здесь теперь должен пропускать любой текст, КРОМЕ команды /cancel и текста кнопки "новый текст"
            GET_TEXT_FOR_CORRECTION: [MessageHandler(filters.TEXT & ~filters.COMMAND & ~filters.Text(["новый текст"]), received_text_for_correction)],
    ...
    # Добавляем обработчик для /cancel и для "новый текст" на глобальном уровне,
    # чтобы они работали даже вне диалога.
    application.add_handler(CommandHandler('cancel', cancel_conversation))
    application.add_handler(MessageHandler(filters.Text(["новый текст"]), request_text_for_correction))
